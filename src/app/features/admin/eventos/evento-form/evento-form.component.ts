...
@Component({
  ...
  template: `
    <div class="max-w-lg">
      <div class="mb-6">
        <a routerLink="/admin/eventos" class="text-sm text-muted hover:text-primary transition-colors">← Volver</a>
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="bg-surface border border-border rounded-xl p-6 space-y-4">
        <app-input label="Nombre del evento" [control]="form.controls.nombre" placeholder="Conferencia Regional…" [required]="true" />
        <app-input label="Fecha y hora" [control]="form.controls.fechaConHora" type="datetime-local" [required]="true" />
        <app-input label="Ubicación (opcional)" [control]="form.controls.ubicacion" placeholder="Nombre de la iglesia, dirección" />
        ...
        <div class="flex gap-3 pt-2">
          ...
          <a routerLink="/admin/eventos"><app-button variant="ghost" type="button">Cancelar</app-button></a>
        </div>
      </form>
    </div>
  `,
...
export class EventoFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id !== null) {
      ...
      this.breadcrumb.set([{ label: 'Admin', route: '/admin/dashboard' }, { label: 'Eventos', route: '/admin/eventos' }, { label: 'Editar' }]);
    } else {
      this.breadcrumb.set([{ label: 'Admin', route: '/admin/dashboard' }, { label: 'Eventos', route: '/admin/eventos' }, { label: 'Nuevo' }]);
    }
  }
  ...
  submit(): void {
    ...
    op.subscribe({
      next: () => { this.loading.set(false); void this.router.navigate(['/admin/eventos']); },
