  ...
  template: `
    <div class="max-w-lg">
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="bg-surface border border-border rounded-xl p-6 space-y-4">
        <app-input label="Nombre" [control]="form.controls.nombre" placeholder="IDDP &quot;Nombre&quot;" [required]="true" />
        <app-input label="Dirección" [control]="form.controls.direccion" placeholder="Jr. Ejemplo 123, Ciudad" [required]="true" />
        <app-select label="Distrito" [control]="form.controls.distritoId" [options]="distritoOptions()" [required]="true" />
        <app-select label="Ministerio (opcional)" [control]="form.controls.ministerioId" [options]="ministerioOptions()" placeholder="Sin ministerio asignado" />
