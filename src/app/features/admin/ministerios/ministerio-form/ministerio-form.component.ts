    ...
    <div class="max-w-lg">
      ...
      >
        <app-input
          ...
          placeholder="Ministerio de la Mujer"
          [required]="true"
        />
        <app-input
          label="Slug (URL)"
        ...
        <div class="flex gap-3 pt-2">
          ...
          <a routerLink="/admin/ministerios"
...
export class MinisterioFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id !== null) {
      ...
      this.service.getById(Number(id)).subscribe((m) => {
        if (m) this.form.patchValue(m);
      });
      this.breadcrumb.set([
        { label: 'Admin', route: '/admin/dashboard' },
        { label: 'Ministerios', route: '/admin/ministerios' },
        { label: 'Editar' },
      ]);
    } else {
      this.breadcrumb.set([
  ...
  submit(): void {
    ...
    op.subscribe({
      next: () => {
        ...
        void this.router.navigate(['/admin/ministerios']);
      },
      error: (err) => {
        this.loading.set(false);
