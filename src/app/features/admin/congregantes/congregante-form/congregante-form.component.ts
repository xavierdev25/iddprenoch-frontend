    ...
    <div class="max-w-2xl">
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-5">
        ...
        <div class="bg-surface border border-border rounded-xl p-6">
          <h2 class="text-xs font-semibold uppercase tracking-widest text-muted mb-4">Datos personales</h2>
          <div class="space-y-4">
            <app-image-upload label="Foto (opcional)" [control]="form.controls.foto" />
            <div class="grid grid-cols-2 gap-4">
              <app-input label="Nombre" [control]="form.controls.nombre" placeholder="Nombre" [required]="true" />
              <app-input label="Apellido" [control]="form.controls.apellido" placeholder="Apellido" [required]="true" />
            </div>
            <div class="grid grid-cols-2 gap-4">
              <app-input label="DNI" [control]="form.controls.dni" placeholder="12345678" [required]="true" />
              <app-select label="Sexo" [control]="form.controls.sexoId" [options]="sexoOptions()" [required]="true" />
            </div>
            <app-select label="Estado civil" [control]="form.controls.estadoCivilId" [options]="estadoCivilOptions()" [required]="true" />
          </div>
        </div>
        ...
        <div class="bg-surface border border-border rounded-xl p-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            <app-input label="Teléfono" [control]="form.controls.telefono" type="tel" placeholder="987 000 000" />
            <app-input label="Correo (opcional)" [control]="form.controls.correo" type="email" placeholder="correo@ejemplo.pe" />
          </div>
        </div>

        <!-- Datos eclesiales -->
        <div class="bg-surface border border-border rounded-xl p-6">
          <h2 class="text-xs font-semibold uppercase tracking-widest text-muted mb-4">Datos eclesiales</h2>
          <div class="space-y-4">
            <app-select label="Iglesia" [control]="form.controls.iglesiaId" [options]="iglesiaOptions()" [required]="true" />
            <app-select label="Estado eclesial" [control]="form.controls.estadoEclesialId" [options]="estadoEclesialOptions()" [required]="true" />
            <div class="grid grid-cols-2 gap-4">
              <app-input label="Fecha de conversión" [control]="form.controls.fechaConversion" type="date" />
              <app-input label="Fecha de bautismo" [control]="form.controls.fechaBautismo" type="date" />
            </div>
          </div>
        </div>
        ...
        <div class="flex gap-3">
          ...
          <a routerLink="/admin/congregantes"><app-button variant="ghost" type="button">Cancelar</app-button></a>
        </div>
      </form>
    </div>
  `,
...
export class CongregantesFormComponent implements OnInit {
  ...
  estadoEclesialOptions = () => this.estadosEclesiales().map(e => ({ value: e.id, label: e.nombre }));
  ...
  ngOnInit(): void {
    ...
    if (id !== null) {
      ...
      this.breadcrumb.set([{ label: 'Admin', route: '/admin/dashboard' }, { label: 'Congregantes', route: '/admin/congregantes' }, { label: 'Editar' }]);
    } else {
      this.breadcrumb.set([{ label: 'Admin', route: '/admin/dashboard' }, { label: 'Congregantes', route: '/admin/congregantes' }, { label: 'Nuevo' }]);
    }
  }
  ...
  submit(): void {
    ...
    const data = stripEmptyStrings({ ...raw, iglesiaId: Number(raw.iglesiaId), sexoId: Number(raw.sexoId), estadoCivilId: Number(raw.estadoCivilId), estadoEclesialId: Number(raw.estadoEclesialId) });
    const id = this.route.snapshot.paramMap.get('id');
    const op = id !== null ? this.service.update(Number(id), data) : this.service.create(data);
    op.subscribe({
      next: () => { this.loading.set(false); void this.router.navigate(['/admin/congregantes']); },
