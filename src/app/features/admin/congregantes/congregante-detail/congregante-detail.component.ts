      ...
      <div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div class="bg-surface border border-border rounded-xl p-5 space-y-3">
            ...
            <dl class="space-y-2 text-sm">
              <div>
                ...
                <dd class="text-foreground font-mono">{{ c.dni }}</dd>
              </div>
              <div>
                <dt class="text-muted">Sexo</dt>
                <dd class="text-foreground">{{ sexo(c.sexoId) }}</dd>
              </div>
              <div>
                <dt class="text-muted">Estado civil</dt>
                <dd class="text-foreground">{{ estadoCivil(c.estadoCivilId) }}</dd>
              </div>
              <div>
                <dt class="text-muted">Teléfono</dt>
                <dd class="text-foreground">{{ c.telefono ?? '—' }}</dd>
              </div>
              <div>
                <dt class="text-muted">Correo</dt>
                <dd class="text-foreground">{{ c.correo ?? '—' }}</dd>
              </div>
            </dl>
          </div>
          ...
          <div class="bg-surface border border-border rounded-xl p-5 space-y-3">
            ...
            <dl class="space-y-2 text-sm">
              <div>
                <dt class="text-muted">Iglesia</dt>
                <dd class="text-foreground">{{ iglesiaName(c.iglesiaId) }}</dd>
              </div>
              <div>
                <dt class="text-muted">Conversión</dt>
                <dd class="text-foreground">{{ c.fechaConversion ?? '—' }}</dd>
              </div>
              <div>
                <dt class="text-muted">Bautismo</dt>
                <dd class="text-foreground">{{ c.fechaBautismo ?? '—' }}</dd>
              </div>
            </dl>
          </div>
        </div>
...
export class CongregantesDetailComponent implements OnInit {
  ...
  iglesiaName(id: number): string {
    return this.iglesias().find((i) => i.id === id)?.nombre ?? '—';
  }
  sexo(id: number): string {
    return this.sexos().find((s) => s.id === id)?.nombre ?? '—';
  }
  estadoCivil(id: number): string {
    return this.estadosCiviles().find((e) => e.id === id)?.nombre ?? '—';
  }
  estadoEclesial(id: number): string {
    return this.estadosEclesiales().find((e) => e.id === id)?.nombre ?? '—';
  }
}
