...
@Component({
  ...
  template: `
    <div class="max-w-lg">
      <div class="mb-6">
        <a routerLink="/admin/lideres" class="text-sm text-muted hover:text-primary transition-colors">← Volver</a>
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="bg-surface border border-border rounded-xl p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <app-input label="Teléfono" [control]="form.controls.telefono" type="tel" placeholder="987 654 321" />
        </div>
        <div class="grid grid-cols-2 gap-4">
        ...
        <app-select label="Ministerio" [control]="form.controls.ministerioId" [options]="ministerioOptions()" [required]="true" />
        ...
        <div class="flex gap-3 pt-2">
          ...
          <a routerLink="/admin/lideres"><app-button variant="ghost" type="button">Cancelar</app-button></a>
        </div>
      </form>
    </div>
  `,
...
export class LiderFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id !== null) {
      ...
      this.service.getById(Number(id)).subscribe(l => { if (l) this.form.patchValue(l); });
      this.breadcrumb.set([{ label: 'Admin', route: '/admin/dashboard' }, { label: 'Líderes', route: '/admin/lideres' }, { label: 'Editar' }]);
    } else {
      this.breadcrumb.set([{ label: 'Admin', route: '/admin/dashboard' }, { label: 'Líderes', route: '/admin/lideres' }, { label: 'Nuevo' }]);
    }
  }
  ...
  submit(): void {
    ...
    const data = stripEmptyStrings({ ...raw, ministerioId: Number(raw.ministerioId), iglesiaId: Number(raw.iglesiaId) });
    const id = this.route.snapshot.paramMap.get('id');
    const op = id !== null ? this.service.update(Number(id), data) : this.service.create(data);
    op.subscribe({
      next: () => { this.loading.set(false); void this.router.navigate(['/admin/lideres']); },
