...
@Component({
  ...
  template: `
    ...
    <section class="relative overflow-hidden pt-[72px] pb-[88px]">
      ...
      <svg
        class="hero-art absolute -right-[60px] -top-[40px] w-[520px] h-[520px] -z-10 opacity-90 min-[860px]:block hidden"
        viewBox="0 0 400 400"
        aria-hidden="true"
      >
      ...
      <div class="relative max-w-6xl mx-auto px-6">
        <div class="flex flex-col min-[860px]:flex-row items-stretch gap-8">
          <div class="max-w-[600px] flex flex-col justify-center items-start">
            ...
            <h1
              class="text-[36px] min-[860px]:text-[52px] leading-[1.15] tracking-[-.01em] text-foreground mb-[18px]"
            >
            ...
            <p class="text-muted text-[17px] leading-relaxed max-w-[480px]">
              Encuentra una iglesia cerca de ti, conoce nuestros ministerios, participa en los
              próximos eventos y sé parte de una comunidad que vive para Cristo.
            </p>
    ...
    <section id="locator" class="bg-foreground text-white py-16">
      <div class="max-w-6xl mx-auto px-6">
        <div class="mb-7">
          ...
          <p class="text-[#B9C0BC] text-sm">
            18 congregaciones en 6 distritos de la Región Norte Chico.
          </p>
        </div>
          ...
          @for (evento of eventos(); track evento.id) {
            <div class="border border-border rounded-[14px] overflow-hidden bg-surface">
              <div class="h-[140px] overflow-hidden">
                @if (evento.imagen) {
                  ...
                  <div
                    ...
                    style="background:linear-gradient(135deg, #0F6B5C, #17493f)"
                  >
                    <span class="text-white/85 font-display italic text-[15px]">{{
                      evento.nombre
                    }}</span>
                  </div>
                }
              ...
              <div class="p-[18px]">
                ...
                <h3 class="text-[17px] font-semibold mt-1.5" style="font-family: var(--font-sans)">
                  {{ evento.nombre }}
                </h3>
    ...
    <section class="py-[72px] bg-surface border-t border-border">
      <div class="max-w-6xl mx-auto px-6">
        <h2 class="text-[30px] text-foreground mb-7">Comunicados</h2>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          @for (c of comunicados(); track c.id) {
            <div
              class="border border-border rounded-lg overflow-hidden bg-surface hover:border-primary/30 transition-colors"
            >
              <app-entity-image
                [src]="c.imagen"
                [alt]="c.titulo"
              ...
              <div class="p-5">
                <h3
                  class="text-[17px] font-semibold text-foreground mb-2"
...
export class HomeComponent {
  ...
  comunicados = toSignal(this.comunicadosService.getAll({ limit: 100 }).pipe(map((r) => r.data)), {
    initialValue: [],
  });
