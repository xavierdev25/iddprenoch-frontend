  ...
  template: `
    <div class="max-w-5xl mx-auto px-4 py-12">
      ...
      @if (obispoRegional(); as jefe) {
        <div class="bg-foreground rounded-xl p-7 mb-8 flex items-center gap-5 relative overflow-hidden">
          <span class="absolute right-4 top-1/2 -translate-y-1/2 font-display text-[90px] leading-none text-white/[.05] select-none" aria-hidden="true">✝</span>
          <div class="w-16 h-16 rounded-full bg-primary flex items-center justify-center shrink-0 relative overflow-hidden">
            @if (jefe.foto) {
              <img [src]="jefe.foto" [alt]="jefe.nombrePastor" class="w-full h-full object-cover cursor-pointer"
            ...
            <div class="bg-surface border border-border rounded-lg p-5">
              <div class="flex items-start justify-between gap-2 mb-3">
                <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden">
                  @if (pastor.foto) {
                    <img [src]="pastor.foto" [alt]="pastor.nombrePastor" class="w-full h-full object-cover cursor-pointer"
              ...
              <h3 class="font-semibold text-foreground text-sm leading-snug">{{ pastor.nombrePastor }}</h3>
...
export class DirectivaComponent {
  ...
  openLightbox(pastor: Pastor): void {
    ...
    this.lightboxImage.set({ src: pastor.foto, alt: pastor.nombrePastor, caption: pastor.nombrePastor });
  }
}
