  ...
  template: `
    <div>
      ...
      <div class="flex flex-wrap gap-2 mb-7">
        <app-chip [active]="selectedDistritoId() === null" (selected)="selectDistrito(null)">
          Todos
        </app-chip>
        @for (d of distritos(); track d.id) {
          <app-chip [active]="selectedDistritoId() === d.id" (selected)="selectDistrito(d.id)">
            {{ d.nombre }}
          </app-chip>
        }
      </div>
      ...
      @if (filteredIglesias().length === 0) {
        <div class="py-12 text-center text-white/60 text-sm">No hay iglesias en este distrito.</div>
      } @else {
        <div
          class="grid grid-cols-1 gap-3.5"
          [class]="compact() ? 'sm:grid-cols-2' : 'sm:grid-cols-2 lg:grid-cols-3'"
        >
          @for (iglesia of filteredIglesias(); track iglesia.id) {
            <div class="bg-[#232B28] border border-white/[.08] rounded-xl overflow-hidden">
              ...
              <div class="p-[18px]">
                <div
                  class="text-[11px] font-semibold text-secondary tracking-[.04em] uppercase mb-2.5"
                >
                  {{ distritoNombre(iglesia.distritoId) }}
                </div>
                <h3
                  class="text-white text-[17px] font-semibold"
